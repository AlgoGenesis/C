...
void insertAtEnd(struct Node** head_ref, int new_data) {
    ...
    // The new node will be the last, so set its next to NULL
    new_node->next = NULL;

    // If the doubly linked list is empty, make the new node the head
    if (*head_ref == NULL) {
        new_node->prev = NULL;  // There's no previous node since it's the only one
        *head_ref = new_node;
        return;
    }
    ...
    last->next = new_node;
    new_node->prev = last;  // Set the previous pointer of the new node to the last node
}
...
void deleteAtEnd(struct Node** head_ref) {
    ...
    // Set the previous node's next to NULL and free the last node
    last->prev->next = NULL;
    free(last);
}
