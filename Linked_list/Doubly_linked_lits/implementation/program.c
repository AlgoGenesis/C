...
int main() {
    // Pointers to the nodes in the doubly linked list
    struct Node* first;    
    struct Node* second;
    struct Node* third;
    struct Node* fourth;

    // Allocate memory for four nodes in the doubly linked list
    ...
    first->prev = NULL;     // First node has no previous node

    second->next = third;   // Second node points to third
    second->prev = first;   // Second node points back to first

    third->next = fourth;   // Third node points to fourth
    third->prev = second;   // Third node points back to second

    fourth->next = NULL;    // Fourth node is the last, so it points to NULL
    fourth->prev = third;   // Fourth node points back to third

    // Call the display function to print the doubly linked list
    display(first);
